  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 cursor-pointer min-w-0" onClick={() => navigate(`/polls/${poll.id}`)}>
        ...
        <div className="flex items-center gap-1 shrink-0">
          {poll.is_active && (
            <button onClick={() => onClose(poll.id)} title="Close poll"
...
export default function MyPollsPage() {
  const navigate = useNavigate();
  const { myPolls, myHasMore, fetchMyPolls, resetMyPolls, deletePoll, closePoll, isLoading } = usePollStore();
    ...
    <div className="min-h-screen pt-20 md:pt-0 pb-16 bg-[#090b10]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 py-6 md:py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
          ...
          <Button leftIcon={<Plus className="w-4 h-4" />} onClick={() => navigate("/create")} className="glow-btn">
            New Poll
          </Button>
        </motion.div>

        {isLoading && myPolls.length === 0 ? (
          ...
          <EmptyState
            ...
            action={<Button leftIcon={<Plus className="w-4 h-4" />} onClick={() => navigate("/create")}>Create your first poll</Button>}
          />
        ) : (
          <InfiniteList onLoadMore={handleLoadMore} hasMore={myHasMore} isLoading={isLoading}>
            <div className="space-y-8">
              {active.length > 0 && (
                <section>
                  <h2 className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-3">Active ({active.length})</h2>
                  <div className="space-y-3">
                    {active.map((p) => <MyPollRow key={p.id} poll={p} onDelete={handleDelete} onClose={handleClose} />)}
                  </div>
                </section>
              )}
              {closed.length > 0 && (
                <section>
                  <h2 className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-3">Closed ({closed.length})</h2>
                  <div className="space-y-3 opacity-70">
                    {closed.map((p) => <MyPollRow key={p.id} poll={p} onDelete={handleDelete} onClose={handleClose} />)}
                  </div>
                </section>
              )}
            </div>
