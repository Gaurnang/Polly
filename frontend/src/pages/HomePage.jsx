...
import { Activity, CheckCircle2, SlidersHorizontal, Users, Filter } from "lucide-react";
...
export default function HomePage() {
  const { polls, nextCursor, hasMore, fetchPolls, resetPolls, isLoading } = usePollStore();
  const [searchParams, setSearchParams] = useSearchParams();

  const filter = searchParams.get("poll_type") || "all";
  const isActiveFilter = searchParams.get("is_active") || "all";

  // Initial load and filter changes — always reset + fetch first page
  useEffect(() => {
    resetPolls();
    const params = {};
  ...
  }, [hasMore, isLoading, nextCursor, filter, isActiveFilter, fetchPolls]);

  const updateFilters = (key, value) => {
    const newParams = { ...Object.fromEntries(searchParams.entries()) };
    if (value === "all") delete newParams[key];
    ...
    <div className="min-h-screen pt-20 md:pt-0 pb-10 bg-[#090b10]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6 md:py-8">
        ...
        {isLoading && polls.length === 0 ? (
          ...
          <InfiniteList onLoadMore={handleLoadMore} hasMore={hasMore} isLoading={isLoading}>
