...
export const createPoll = async ({ question, poll_type, options }, creator_id) => {
  if (!question || !question.trim()) throw { status: 400, message: "Poll question is required." };
  if (!VALID_POLL_TYPES.includes(poll_type))
    throw { status: 400, message: `poll_type must be one of: ${VALID_POLL_TYPES.join(", ")}.` };
  if ((poll_type === "single" || poll_type === "single_choice") && (!Array.isArray(options) || options.length < 2))
    throw { status: 400, message: "Choice polls require at least 2 options." };

  const poll = await createPollRecord({ question: question.trim(), poll_type, creator_id });
  if (Array.isArray(options) && options.length > 0) {
    for (const optionText of options) await insertPollOption(poll.id, optionText);
  }
  return await findPollById(poll.id);
};
...
export const updatePoll = async (id, updates, requesterId) => {
  ...
  if (Array.isArray(options)) {
    ...
    for (const optionText of options) await insertPollOption(id, optionText);
  }
...
export const closePoll = async (id, requesterId) => {
  ...
  if (String(poll.creator_id) !== String(requesterId))
    throw { status: 403, message: "Forbidden: you are not the poll creator." };
  if (!poll.is_active) throw { status: 400, message: "Poll is already closed." };
  return await closePollRecord(id);
};

export const getMyPolls = async (creator_id, filters = {}) => {
  return await findPollsByCreator(creator_id, filters);
};

export const getMyVotedPolls = async (user_id, filters = {}) => {
  return await findPollsVotedByUser(user_id, filters);
};
